// your answers
type IsIncludesZero<T> = [Extract<T, 0 | '0' | 0n>] extends [never] ? false : true
...
> = `${A}` extends `${infer X extends number}`
  ...
  : never
type dd = MultiplyWithSmallNumber<2,3>
type ReverseString<S extends string> = S extends `${infer F}${infer tail}` ? `${ReverseString<tail>}${F}`: ''
...
type cc = IsIncludesZero<0n>
type c = '123'extends `${infer R}${infer F extends number}`? F|R: never
