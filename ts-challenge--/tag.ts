...
type StartsWith<T extends any[], K extends any[]> = T extends [
  infer T0,
  ...infer Tr
]
...
type SuccessionWith<T extends any[], K extends any[]> = T extends [
  infer T0,
  ...infer Tr
]
  ? K extends [infer K0, ...infer Kr]
    ? Is<T0, K0> extends true
      ? StartsWith<Tr, Kr> extends true
        ? true
        : SuccessionWith<Tr, K>
      : SuccessionWith<Tr, K>
    : [] extends K
...
type SuccessionWithTest = [
  ...
  Expect<Equal<SuccessionWith<[2, 0, 2, 1], [2, 1]>, true>>,
 ...
 */
type SetTags<B, T extends string[]> = IsNever<B> extends true
  ? Tags<T, B>
 ...
 */
type GetTagTupples<B> = IsNever<Exclude<B, GetTagged<B>>> extends true
...
type d = Tag<any, "foo"> | 1
type c<T> = IsNever<Exclude<T, GetTagged<T>>>;
type cc<B> = B extends Tags<infer T, never>
? T
: [];
type cccc = c<d>
type ccc = cc<d>
type ccccc = GetTagged<d>
type UnTag<B> = B extends infer X
  ? X extends Tags<infer _, infer K>
    ? K
    : X
  : never;
...
type HasTags<B, T extends readonly string[]> = SuccessionWith<
  GetTags<B>,
  [...T]
> extends true
  ? true
...
type cases = [
  ...
  Expect<
    Equal<
      HasTags<
        Tag<Tag<Tag<Tag<{}, "baz">, "foo">, "foo">, "bar">,
        ["foo", "bar"]
      >,
      true
    >
...
type a1 = a<1>; // 1
type a2 = a<"1">; // '1'
type a3 = a<false>; // false
type a4 = a<never>; // never
type a5 = a<void>; // void & {}
type a6 = a<undefined>; // never
type a7 = a<null>; // never
type a8 = a<unknown>; // {}

type b<T> = T & [];
type b1 = b<1>; // 1 & []
type b2 = b<"1">; // '1' & []
type b3 = b<false>; // false & []
type b4 = b<never>; // never
type b5 = b<void>; // void & []
type b6 = b<undefined>; // never
type b7 = b<null>; // never
type b8 = b<unknown>; // []
type b9 = b<{ 2: 3 }>; // {2: 3} & []
