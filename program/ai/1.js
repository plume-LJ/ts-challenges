...
// Set up the initial position and radius of the hearts
...
function draw() {
  ...
  t1 += 0.05;
  x1 = canvas.width / 3 + 50 * (2 * Math.sin(t1) ** 3);
  y1 = canvas.height / 2 - 40 * (Math.cos(t1) - Math.sin(t1) ** 2);
  radius1 = 100 + 20 * Math.sin(t1);
  scale1 = 0.8 + 0.4 * Math.abs(Math.sin(t1));

  // Update the position, radius, and scale of the second heart
  t2 += 0.05;
  x2 = (canvas.width * 2) / 3 + 50 * (2 * Math.sin(t2) ** 3);
  y2 = canvas.height / 2 - 40 * (Math.cos(t2) - Math.sin(t2) ** 2);
  radius2 = 100 + 20 * Math.sin(t2);
  scale2 = 0.8 + 0.4 * Math.abs(Math.sin(t2));
  ...
  // Translate and scale the first heart
  ctx.translate(x1, y1);
  ctx.scale(scale1, scale1);

  // Draw the first heart
  const firstHeartGradient = ctx.createRadialGradient(
    0,
    radius1 / 4,
    0,
    0,
  ...
  // Save the current state of the context
  ctx.save();
  // Translate and scale the second heart
  ctx.translate(x2, y2);
  ctx.scale(scale2, scale2);

  // Draw the second heart
  const secondHeartGradient = ctx.createRadialGradient(
    0,
    radius2 / 4,
    0,
    0,
