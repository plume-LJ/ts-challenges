function solveNQ(n: number): string[][] {
  const board = Array.from({ length: n }, () =>
    Array.from({ length: n }, () => ".")
  );
  const colSet = new Set<number>();
  const helperSet1 = new Set<number>();
  const helperSet2 = new Set<number>();

  const resArr: string[][] = [];

  function backTracking(row: number, path: string[]) {
    if (row === n) {
      resArr.push(board.map((arr) => arr.join("")));
    }
    ...
    for (let col = 0; col < n; col++) {
      ...
      if (helperSet1.has(tmp1)) continue;
      if (helperSet2.has(tmp2)) continue;
      colSet.add(col);
      helperSet1.add(tmp1);
      helperSet2.add(tmp2);
      board[row]![col] = "Q";
