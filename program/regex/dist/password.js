...
var re1 = /^(?![\d]+$)(?![a-z]+$)(?![A-Z]+$)(?![!#$%^&*]+$)[\da-zA-z!#$%^&*]{8,16}$/;
// 密码中允许出现数字、大写字母、小写字母、特殊字符（!#$%^&*），但至少包含其中3种且长度在8-16之间（四种符号任取其三）
// 正向否定方式：
var ree = /^(?![\da-z]+$)(?![\dA-Z]+$)(?![\d!#$%^&*]+$)(?![a-zA-Z]+$)(?![a-z!#$%^&*]+$)(?![A-Z!#$%^&*]+$)[\da-zA-z!#$%^&*]{8,16}$/;
// 先行断言方式：
var ree1 = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[\da-zA-Z!#$%^&*]{8,16}$/;
var ree2 = /^(?=.*\d)(?=.*[a-z])(?=.*[!#$%^&*])[\da-zA-Z!#$%^&*]{8,16}$/;
var ree3 = /^(?=.*\d)(?=.*[A-Z])(?=.*[!#$%^&*])[\da-zA-Z!#$%^&*]{8,16}$/;
var ree4 = /^(?=.*[a-z])(?=.*[A-Z])(?=.*[!#$%^&*])[\da-zA-Z!#$%^&*]{8,16}$/;
// 数字 /\d/
// 字母 /[a-z]/
// 数字 /[A-Z]/
// 数字 /[^\da-zA-Z]/
